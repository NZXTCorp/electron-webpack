...
export class BaseRendererTarget extends BaseTarget {
  ...
  configureRules(configurator: WebpackConfigurator): void {
    ...
    const cssHotLoader = configurator.isProduction
      ? miniLoaders
  ...
  async configurePlugins(configurator: WebpackConfigurator): Promise<void> {
    ...
    configurator.plugins.push(
      new MiniCssExtractPlugin({
        filename: `${
          configurator.type === "renderer-dll" ? "vendor" : "styles"
        }.css`,
      })
    );
...
export class RendererTarget extends BaseRendererTarget {
  ...
  async configurePlugins(configurator: WebpackConfigurator): Promise<void> {
    ...
    const customTemplateFile = path.join(
      configurator.projectDir,
      configurator.rendererTemplate
    );
    const HtmlWebpackPlugin = require("html-webpack-plugin");
    const nodeModulePath = configurator.isProduction
      ? null
      : path.resolve(require.resolve("electron"), "..", "..");
    ...
    configurator.plugins.push(
      new HtmlWebpackPlugin({
        filename: "index.html",
        template: await generateIndexFile(
          configurator,
          nodeModulePath,
          template
        ),
    ...
    if (configurator.isProduction) {
      ...
      const contentBase = [
        path.join(configurator.projectDir, configurator.staticSourceDirectory),
    ...
    await BaseRendererTarget.prototype.configurePlugins.call(
      this,
      configurator
    );
  }
